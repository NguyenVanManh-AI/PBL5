...
pathTrain = "../LBPLearning/CSDLNew/HuanLuyenNew/"
images = []
classnames = []
...
for cl in myList:
    print(cl)
    curImg = cv2.imread(f"{pathTrain}/{cl}", cv2.IMREAD_GRAYSCALE)
...
def Mahoa (images):
    ...
    for img, index in zip(images, range(len(images))):
        ...
        if len(locationFace) != 0:
            ...
            cv2.imwrite(f"../LBPLearning/LBPImage/{myList[index]}", out_our)
...
df.to_csv("../LBPLearning/SaveDir/know_face_encode.csv", index=False)
dfname.to_csv("../LBPLearning/SaveDir/know_face_classnames.csv", index=False)