...
def main():
    ...
    img = cv2.imread('../LBPLearning/CSDLNew/HuanLuyenNew/HL_Cuong_ (11).JPG', cv2.IMREAD_GRAYSCALE)
    locationFace = face_recognition.face_locations(img)[0]
    y1, x2, y2, x1 = locationFace
    cropped_img = img[y1:y2, x1:x2]
    down_points = (64, 64)
    resize_down = cv2.resize(cropped_img, down_points, interpolation=cv2.INTER_LINEAR)
    out_scikit = local_binary_pattern(image=resize_down, P=8, R=1, method='default')
    print("Scikit output:", out_scikit)
    cv2.imwrite('lbp_phuc_lib.jpg', out_scikit)
    print("Saved image @ lbp_phuc_lib.jpg")

    lbp = LBP()
    out_our = lbp(resize_down)
    cv2.imwrite('lbp_phuc_our.jpg', out_our)
    print("Saved image @ lbp_phuc_our.jpg")
    print("Our output:", out_our)
    print("Same output:", (out_our == out_scikit).all())

    # đã có LBP_img
    hist, _ = np.histogram(out_our, bins=128)
    print(hist)
    # chuan hoa
