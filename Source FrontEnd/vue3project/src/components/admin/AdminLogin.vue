<template>
    <div class="blue col-4 mx-auto adminlogin">
        <form @submit.prevent="login()" class="col-12">
            <div class="row">
                <h1 class="col-12 text-center"><i class="fa-solid fa-user-shield"></i> Admin Login</h1>
            </div>
            <br>
            <br>
            <div class="form-group row">
                <label for="exampleInputEmail1"><i class="fa-solid fa-envelope"></i> Email</label>
                <input type="email" required v-model="admin.email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
            </div>
            <div class="form-group row">
                <label for="exampleInputPassword1"><i class="fa-solid fa-key"></i> Password</label>
                <input required v-model="admin.password" type="password" class="form-control" id="exampleInputPassword1">
            </div>
            ...
            .then(async (querySnapshot) => {
                if (querySnapshot.size > 0) {
                    for (const doc of querySnapshot.docs) {
                        ...
                        const hashedPassword = admin.password; // hashedPassword được lấy ra từ firestore
                        const isMatch = await bcrypt.compare(this.admin.password, hashedPassword);
                        if (isMatch) {
                            ...
                            window.localStorage.setItem('admin',JSON.stringify(system_admin));
                            setTimeout(() => {
                                this.$router.push({name:'AdminProfile'}); 
                            }, 1000);
                        } else {
